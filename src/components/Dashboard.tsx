...
export default function Dashboard({ cases, items, lang, onSync, syncing }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Recent activity */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">        <h2 className="font-semibold text-gray-700 mb-4">{t('recentActivity', lang)}</h2>
        {recent.length === 0 ? (
              ...
              <div key={item.id} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {item.amount && (
                      <span className="text-xs text-red-600 font-medium">₹{item.amount.toLocaleString('en-IN')}</span>
                    )}
                  </div>
                </div>
